        ...
        <div class="breadcrumb-outer">
            <div class="container">
                <div class="breadcrumb-content text-center">
                    <h1 class="mb-3">Potensi Wisata</h1>
                    <nav aria-label="breadcrumb" class="d-block">
                        <ul class="breadcrumb">
                            <li class="breadcrumb-item"><a href="{{ url('/') }}">Home</a></li>
                            <li class="breadcrumb-item active" aria-current="page">Potensi Wisata</li>
                        </ul>
                    </nav>
                </div>
            </div>
    ...
    <section class="trending pb-9">
        <div class="container">
            ...
            <div class="section-title mb-6 w-75 mx-auto text-center">
                <h4 class="mb-1 theme1">Potensi Wisata</h4>
                @if (isset($key))
                    <h2 class="mb-1">Daftar Desa Wisata</h2>
                    <p>Hasil Pencarian Untuk : <span class="theme">{{ $key }}</span></p>
                @else
                    <h2 class="mb-1">Potensi Wisata <span class="theme">{{ ucwords($objek_wisata->nama) }}</span></h2>
                @endif
                    ...
                    @if ($datas->count()>0)
                        @foreach ($datas as $item)
                            <div class="col-lg-4 mb-4">
                                <a href="{{ route('detailPotensi',$item->id) }}">
                                    <div class="trend-item1 rounded box-shadow bg-white">
                                        <div class="trend-image position-relative">
                                            @foreach ($item->potensi_images as $image)
                                            @endforeach
                                            <img src="{{ asset('images/potensi/'.$image->folder.'/'.$image->name) }}" style="width: 100%;height:400px;object-fit:cover;" alt="image" class="">
                                            <div class="trend-content1 p-4">
                                                <p class="mb-1 fw-bold white">{{ ucwords($item->jenis_potensi->nama_jenis_potensi) }}</p>
                                                <h3 class="mb-1 white">{{ ucwords($item->nama) }}</h3>
                                                <div class="entry-meta d-flex align-items-center justify-content-between">
                                                    <div class="entry-author">
                                                        <i class="fas fa-map-marker-alt theme1"></i>
                                                        <span class="theme1"> {{ ucwords($item->alamat) }}</span>
                                                    </div>
                                                </div>
                                            </div>
